...
(function (j$) {
   ...
   */
  j$.uri = function (relative, base) {
    var uri;
    relative = relative || '';
    if (mem[relative]) {
      return mem[relative];
    }
    base = base || j$.uri.base();
    if (typeof base === 'string') {
      base = j$.uri.absolute(base);
    }
    uri = new j$.uri.fn.init(relative, base);
    if (mem[uri]) {
      return mem[uri];
    } else {
      mem[uri] = uri;
  ...
  j$.uri.fn = j$.uri.prototype = {
    ...
    init: function (relative, base) {
      var r = {};
      base = base || {};
      j$.extend(this, parseURI(relative));
  ...
  j$.uri.fn.init.prototype = j$.uri.fn;
   ...
   * @see jQuery.uri
   */
  j$.uri.resolve = function (relative, base) {
    return j$.uri(relative, base);
  };
   ...
   * @returns {String}
   */
  j$.uri.relative = function (absolute, base) {
    return j$.uri(base, {}).relative(absolute);
  };
   ...
   * @name jQuery#base
   * @returns {jQuery.uri}
   * @example baseURI = j$('img').base();
   */
  j$.fn.base = function () {
    var base = j$(this).parents().andSelf().find('base').attr('href'),
      doc = j$(this)[0].ownerDocument || document,
      docURI = j$.uri.absolute(doc.location === null ? document.location.href : doc.location.href);
    return base === undefined ? docURI : j$.uri(base, docURI);
  };
