...
export async function runMcpServer(): Promise<void> {
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      ...
      if (!snippet) {
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: `Snippet not found: ${id}` }) }]
        };
      }
      return {
        content: [
  ...
  server.registerTool(
    ...
    {
      description:
        "Get a formatted prompt for a snippet (markdown with HTML and JSX blocks, token estimate) for pasting into AI tools.",
      inputSchema: {
        id: z.string().describe("Snippet ID from list_snippets")
      }
    },
    async ({ id }) => {
      ...
      if (!snippet) {
        return {
          content: [{ type: "text" as const, text: `Snippet not found: ${id}` }]
        };
      }
