...
> **Superseded by** `plans/active/2026-05-17-auto-blogger-lambda-migration-and-reusable-package.md`. Phase 1 of this plan (swap + `mem_limit` changes) remains in effect and benefits the remaining `server`, `mcp`, and `nginx` containers. Phase 2 (systemd timer install) has been rolled back and replaced by the Lambda deployment in the superseding plan.
