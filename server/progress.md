...
- **Sign-in**: User opens sign-in page; Google OAuth starts via `POST /api/auth/sign-in/social` with `{ provider: 'google', callbackURL: '/account.html' }`, then redirect to the `Location` response header URL. Magic-link: form POSTs to `/api/auth/sign-in/magic-link` with `{ email, callbackURL }` (Better Auth). Callback URL should point to account (or a page that then links install if `install_id` in query).
